...
export default function CatalogScreen({ navigation, route }) {
  ...
  return <View className="flex-1 bg-canvas"><Header title="Catálogo" subtitle={`${filtered.length} productos disponibles`} />
    ...
    <FlatList data={filtered} numColumns={2} keyExtractor={(item) => String(item.id)} columnWrapperStyle={{ justifyContent: "space-between" }} contentContainerStyle={{ paddingHorizontal: 18, paddingBottom: 100 }} ListEmptyComponent={<View className="items-center px-8 py-16"><Text className="text-xl font-black text-content">Sin productos en {selected}</Text></View>} renderItem={({ item }) => <ProductCard product={item} onPress={() => navigation.navigate("Producto", { product: item, productId: item.id })} />} />
  </View>;
}
