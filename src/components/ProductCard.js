  ...
  return (
    ...
    >
      ...
      <View className="p-3.5">
        <Text className="text-[10px] font-bold uppercase tracking-widest text-accent">{product.category} · {product.volume}</Text>
        <Text numberOfLines={2} className="mt-1 min-h-[40px] text-sm font-extrabold leading-5 text-content">{product.name}</Text>
        <Text numberOfLines={1} className="mt-1 text-[10px] text-muted">{product.brand} · {product.country}</Text>
        <View className="mt-2 flex-row items-center"><Text className="text-xs text-accent">★ {product.rating}</Text><Text className="ml-2 text-[10px] text-subtle">({product.reviews})</Text></View>
